...
namespace nxpbc {
  ...
  NxpImage::NxpImage(uint16_t (&rawImage)[CAMERA_LINE_LENGTH]) : rawImage_{0},
                                                                 normalizedImage_{0},
                                                                 thresholdedImage_{0},
                                                                 max_{COLOR_BLACK},
                                                                 min_{COLOR_WHITE} {
  ...
  void NxpImage::cut(uint16_t (&srcImg)[CAMERA_LINE_LENGTH]) {
    for (int col = 0; col < BLACK_COUNT; col++) {
      //srcImg[col] = max_;
      srcImg[col] = srcImg[BLACK_COUNT];
    }
    
    for (int col = CAMERA_LINE_LENGTH - 1; col > CAMERA_LINE_LENGTH - 1 - BLACK_COUNT; col--) {
      //srcImg[col] = max_;
      srcImg[col] = srcImg[CAMERA_LINE_LENGTH - 1 - BLACK_COUNT];
    }
  ...
  void NxpImage::normalize(const uint16_t (&srcImg)[CAMERA_LINE_LENGTH], uint16_t (&dstImg)[CAMERA_LINE_LENGTH]) {
    if ((max_ - min_) == 0) {
      std::cout << "WARNING: min: " << min_ << " max: " << max_ << std::endl;
      return;
    }
  ...
  uint16_t NxpImage::getAverageThreshold(const uint16_t (&srcImg)[CAMERA_LINE_LENGTH]) {
    ...
    uint16_t avg = static_cast<uint16_t>(sum / CAMERA_LINE_LENGTH);
    return avg;
  }
  ...
  uint16_t NxpImage::at(const uint8_t index, ImgType type) const {
    if (index >= 128) {
      printf("NXP image index mimo pole\n");
    }
    switch (type) {
      case ImgType::Raw:return rawImage_[index];
      case ImgType::Normalized:return normalizedImage_[index];
  ...
  void NxpImage::printImg(ImgType type) {
    switch (type) {
      case ImgType::Raw:printImg(rawImage_);
        break;
      case ImgType::Normalized:printImg(normalizedImage_);
        break;
      case ImgType::Thresholded:printImg(thresholdedImage_);
        break;
    }
